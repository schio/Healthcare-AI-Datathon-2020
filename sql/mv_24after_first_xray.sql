...
xray_ts as (
	select
		subject_id ,
		-- study_id ,
		-- min(to_date("StudyDate"::text, 'YYYYMMDD')) as ts
		min(to_timestamp(
			case when "StudyTime" < 100000 then "StudyDate"::text || '0' || trim(from to_char(floor("StudyTime"/10000)*10000, '999999D'))
			else "StudyDate"::text || to_char(floor("StudyTime"/10000)*10000, '999999D')
			end 
		, 'YYYYMMDDHH24MISS')) as first_xray_ts
	from
		mimic_cxr.metadata m
		group by subject_id 
)
select x.first_xray_ts, m.*
from mv_ts m
join xray_ts x using (subject_id)
where m.charttime - interval '24 hours' >= x.first_xray_ts;
-- mv_count = 480,025, p_count = 8,894
